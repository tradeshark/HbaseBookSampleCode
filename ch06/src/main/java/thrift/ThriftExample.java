...
public class ThriftExample {
  ...
  public static void main(String[] args) throws IOException {
    try {
      TTransport transport = new TSocket("192.168.0.150", 9090, 20000);
      ...
      mutations.add(new Mutation(false, COLUMN,
        VALUE));
      client.mutateRow(TABLE, ROW,
        mutations);

      ArrayList<byte[]> columnNames = new ArrayList<byte[]>();
      columnNames.add(FAMILY2);
      int scannerId = client.scannerOpen(TABLE, null,
        null);
