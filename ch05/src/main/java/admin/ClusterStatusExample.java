...
public class ClusterStatusExample {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    for (ServerName server : status.getServers()) { // co ClusterStatusExample-2-ServerInfo Iterate over the included server instances.
      ...
      System.out.println("\nRegion Load:\n--------------");
      for ( HServerLoad.RegionLoad entry : // co ClusterStatusExample-4-Regions Iterate over the region details of the current server.
        load.getRegionsLoad()) {
       // System.out.println("Region: " + Bytes.toStringBinary(entry.getKey()));

        HServerLoad.RegionLoad regionLoad = entry; // co ClusterStatusExample-5-RegionLoad Get the load details for the current region.
